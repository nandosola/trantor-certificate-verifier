...
import cc.abstra.trantor.security.certificate.AbstractCertStatus;
import cc.abstra.trantor.security.certificate.RevokedInfo;
...
public class OCSPStatus extends AbstractCertStatus implements IOCSPCertStatus {
	...
	public OCSPStatus(OCSPResponse resp, X509Certificate cert) {
		...
		if (resp.getNroRespuesta() == OCSPConstants.GOOD) {
			setCertStatus(CERT_STATUS.valid);
		} else if (resp.getNroRespuesta() == OCSPConstants.REVOKED) {
